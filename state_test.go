package engineio

import (
	"bytes"
	"encoding/json"
	"testing"

	"github.com/sarvalabs/go-polo"
	"github.com/stretchr/testify/require"
	"gopkg.in/yaml.v3"
)

func TestStateMatrix(t *testing.T) {
	tests := []struct {
		object     StateMatrix
		persistent bool
		ephemeral  bool
	}{
		{StateMatrix{PersistentState: 0, EphemeralState: 1}, true, true},
		{StateMatrix{PersistentState: 0}, true, false},
		{StateMatrix{EphemeralState: 1}, false, true},
		{StateMatrix{}, false, false},
	}

	for _, test := range tests {
		require.Equal(t, test.persistent, test.object.Persistent())
		require.Equal(t, test.ephemeral, test.object.Ephemeral())
	}
}

func TestStateKind_String(t *testing.T) {
	require.True(t, len(stateKindFromString) == len(stateKindToString))

	require.Equal(t, "persistent", PersistentState.String())
	require.Equal(t, "ephemeral", EphemeralState.String())

	require.PanicsWithValue(t, "unknown StateKind variant", func() {
		_ = StateKind(10).String()
	})
}

//nolint:dupl
func TestStateKind_Serialization(t *testing.T) {
	t.Run("POLO", func(t *testing.T) {
		for kind, str := range stateKindToString {
			encoded, err := polo.Polorize(kind)

			require.Nil(t, err)
			require.Equal(t, bytes.Join([][]byte{{6}, []byte(str)}, []byte{}), encoded)

			decoded := new(StateKind)
			err = polo.Depolorize(decoded, encoded)

			require.Nil(t, err)
			require.Equal(t, kind, *decoded)
		}

		require.PanicsWithValue(t, "unknown StateKind variant", func() {
			_, _ = polo.Polorize(StateKind(10))
		})

		malformed := bytes.Join([][]byte{{6}, []byte("malformed")}, []byte{})
		err := polo.Depolorize(new(StateKind), malformed)
		require.EqualError(t, err, "invalid StateKind value")
	})

	t.Run("JSON", func(t *testing.T) {
		for kind, str := range stateKindToString {
			encoded, err := json.Marshal(kind)

			require.Nil(t, err)
			require.Equal(t, bytes.Join([][]byte{{34}, []byte(str), {34}}, []byte{}), encoded)

			decoded := new(StateKind)
			err = json.Unmarshal(encoded, decoded)

			require.Nil(t, err)
			require.Equal(t, kind, *decoded)
		}

		require.PanicsWithValue(t, "unknown StateKind variant", func() {
			_, _ = json.Marshal(StateKind(10))
		})

		malformed := bytes.Join([][]byte{{34}, []byte("malformed"), {34}}, []byte{})
		err := json.Unmarshal(malformed, new(StateKind))
		require.EqualError(t, err, "invalid StateKind value")
	})

	t.Run("YAML", func(t *testing.T) {
		for kind, str := range stateKindToString {
			encoded, err := yaml.Marshal(kind)

			require.Nil(t, err)
			require.Equal(t, bytes.Join([][]byte{[]byte(str), {10}}, []byte{}), encoded)

			decoded := new(StateKind)
			err = yaml.Unmarshal(encoded, decoded)

			require.Nil(t, err)
			require.Equal(t, kind, *decoded)
		}

		require.PanicsWithValue(t, "unknown StateKind variant", func() {
			_, _ = yaml.Marshal(StateKind(10))
		})

		malformed := bytes.Join([][]byte{{34}, []byte("malformed"), {34}}, []byte{})
		err := yaml.Unmarshal(malformed, new(StateKind))
		require.EqualError(t, err, "invalid StateKind value")
	})
}
